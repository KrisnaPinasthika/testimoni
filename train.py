...
from loss import SILogLoss, DepthSmoothness, L1Loss, LossSSIM, BerHuLoss
from utils import RunningAverage, colorize
import matplotlib

PROJECT = "New U-Net MGA"
logging = True
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = models.UNetSWCA(
        device='cuda', 
        input_hw=explicit_hw,
        backbone_name='eff_v2_m', 
        window_sizes=window_sizes,
        swca_blocks=[2, 2, 2]
    )
    
    # model = models.PreTrainedUNetSWCA(
    #     device='cuda',
...
def train(model, args, epochs=10, experiment_name="test-01x", lr=0.0001, root=".", device=None, optimizer_state_dict=None):
    global PROJECT
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            loss_silog = criterion_silog(pred, depth, mask=mask, interpolate=False)
            loss_l1 = criterion_l1(pred, depth, mask=mask, interpolate=False)   
            # loss_ds = criterion_ds(pred, depth, mask=mask_ds, interpolate=False)
            ...
            optimizer.step()
            if step % 5 == 0:
                wandb.log({f"Train/{criterion_l1.name}": loss_l1.item()}, step=step)
                wandb.log({f"Train/{criterion_silog.name}": loss_silog.item()}, step=step)
            ...
            if should_write and step % args.validate_every == 0:
                ...
                # if should_log:
                wandb.log({f"Test/DoubleLoss": val_si.get_value(),}, step=step)
...
def validate(args, model, test_loader, criterions, epoch, epochs, device='cpu'):
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Validation") if is_rank_zero(
            ...
            loss_l1 = criterions[1](pred, depth, mask=mask, interpolate=False)    
            # loss_ds = criterions[2](pred, depth, mask=mask_ds, interpolate=False)
            
            loss = loss_l1 + loss_silog # + loss_ds            
